...
class KOOWEXAPoster {
  ...
  setupEventListeners() {
    ...
    if (this.downloadImageBtn)
      this.downloadImageBtn.addEventListener(
        "click",
  ...
  updateZoomButtons() {
    if (this.zoomInBtn)
      this.zoomInBtn.disabled = this.currentScale >= this.maxScale;
    if (this.zoomOutBtn)
      this.zoomOutBtn.disabled = this.currentScale <= this.minScale;
  }
  ...
  endDrag() {
    ...
    if (this.imageElement) {
      this.imageElement.style.cursor =
        this.currentScale > 1 ? "grab" : "default";
    }
  }
