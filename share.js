...
class KOOWEXAShare {
    ...
    setupShareButtons() {
        document.addEventListener('click', (e) => {
            if (e.target.id === 'shareWhatsapp' || e.target.closest('#shareWhatsapp')) {
                e.preventDefault();
                this.share('whatsapp');
            } else if (e.target.id === 'shareTelegram' || e.target.closest('#shareTelegram')) {
                e.preventDefault();
                this.share('telegram');
            } else if (e.target.id === 'shareFacebook' || e.target.closest('#shareFacebook')) {
                e.preventDefault();
                this.share('facebook');
            } else if (e.target.id === 'shareTwitter' || e.target.closest('#shareTwitter')) {
                e.preventDefault();
                this.share('twitter');
            } else if (e.target.id === 'shareEmail' || e.target.closest('#shareEmail')) {
                e.preventDefault();
                this.share('email');
    ...
    share(platform) {
        ...
        if (shareUrl) {
            ...
            if (window.KOOWEXA_API && window.KOOWEXA_API.getAPI('analytics')) {
                window.KOOWEXA_API.getAPI('analytics').trackEvent('Share', platform, 'Platform Share');
            }
        }
    }
