...
class KOOWEXAAPIs {
  ...
  logger = {
    info: (message, data = {}) =>
      console.log(`[KOOWEXA-INFO] ${message}`, data),
    error: (message, error = null) =>
      console.error(`[KOOWEXA-ERROR] ${message}`, error),
    warn: (message, data = {}) =>
      console.warn(`[KOOWEXA-WARN] ${message}`, data),
    debug: (message, data = {}) =>
      console.debug(`[KOOWEXA-DEBUG] ${message}`, data),
  };
  ...
  setupEventListeners() {
    ...
    window.addEventListener("beforeunload", (event) => {
      ...
      if (hasQueuedData && !this.state.online) {
        ...
        event.returnValue =
          "Tienes datos pendientes de sincronizar. ¿Estás seguro de que quieres salir?";
      }
    });
  }
  ...
  initPaymentAPI() {
    ...
    const createPayment = async (orderData) => {
      ...
      try {
        const response = await fetch(
          `${baseURLs.payment}${endpoints.createPayment}`,
          {
            method: "POST",
            headers: {
        ...
        this.apis.analytics.trackEvent(
          "Payment",
          "Created",
    ...
    const verifyPayment = async (paymentId) => {
      try {
        const response = await fetch(
          `${baseURLs.payment}${endpoints.verifyPayment}/${paymentId}`,
          {
            headers: {
              Authorization: `Bearer ${getAuthToken("payment")}`,
            },
    ...
    const getWalletBalance = async () => {
      try {
        const response = await fetch(
          `${baseURLs.payment}${endpoints.getBalance}`,
          {
            headers: {
              Authorization: `Bearer ${getAuthToken("payment")}`,
            },
  ...
  initNotificationAPI() {
    ...
    const sendSMS = async (notification) => {
      ...
      try {
        ...
        this.logger.warn("SMS en cola para envío posterior", {
          error: error.message,
        });
    ...
    const processQueuedNotifications = async () => {
      ...
      for (const notification of queued) {
        if (notification.attempts < this.config.defaults.maxRetries) {
          ...
          if (result.success) {
            // Remover de la cola
            const updatedQueue = queued.filter(
  ...
  initAnalyticsAPI() {
    ...
    const storeInSession = (eventData) => {
      const sessionEvents = JSON.parse(
        sessionStorage.getItem("koowexa_session_events") || "[]",
      );
      sessionEvents.push(eventData);
      sessionStorage.setItem(
        "koowexa_session_events",
    ...
    const syncOfflineData = async () => {
      ...
      this.logger.info(
        `Sincronizando ${offlineData.length + criticalData.length} eventos de analytics`,
      );
    ...
    const getReports = () => {
      ...
      return {
        ...
        conversions: allEvents.filter((e) => e.category === "Conversion")
          .length,
      ...
      try {
        ...
        socket.onclose = (event) => {
          ...
          if (reconnectAttempts < maxReconnectAttempts) {
            ...
            this.logger.error(
              "Máximo de reconexiones alcanzado, usando polling",
            );
            startPolling();
          }
        };
        ...
        this.logger.error(
          "Error inicializando WebSocket, usando polling",
          error,
        );
        startPolling();
      }
    ...
    const getCurrentLocation = () => {
      return new Promise((resolve) => {
        ...
        navigator.geolocation.getCurrentPosition(
          ...
          (error) => {
            this.logger.warn(
              "GPS falló, usando IP o ubicación guardada",
              error,
            );
  ...
  initValidationAPI() {
    ...
    const validateProductData = (product) => {
      ...
      if (!product.price || !validateCUPAmount(product.price)) {
        errors.push(
          "El precio debe ser un valor válido en CUP entre 1 y 1,000,000",
        );
      }
      ...
      if (
        product.category &&
        !["tecnologia", "hogar", "ropa", "alimentos", "otros"].includes(
          product.category,
        )
      ) {
    ...
    const validateStoreData = (store) => {
      ...
      if (store.description && store.description.length > 500) {
        errors.push(
          "La descripción del negocio no puede exceder 500 caracteres",
        );
      }
  ...
  initSupportAPI() {
    const { baseURLs } = this.config;
    const { generateId, queueData, getQueuedData, clearQueuedData } =
      this.utils;
      ...
      const articles = [
        {
          ...
          category: "tienda",
          content:
            "Guía paso a paso para configurar tu tienda online con KOOWEXA. Desde el registro hasta la publicación de productos.",
        ...
        },
        {
          ...
          category: "tienda",
          content:
            "Aprende a agregar, editar y administrar productos en tu tienda. Control de inventario y variantes.",
        ...
        },
        {
          ...
          category: "pagos",
          content:
            "Cómo configurar y gestionar los diferentes métodos de pago disponibles para tu tienda.",
        ...
        },
        {
          ...
          category: "tecnico",
          content:
            "Soluciones para problemas comunes de conectividad y acceso a la plataforma.",
        ...
        },
        {
          ...
          category: "tienda",
          content:
            "Mejores prácticas para optimizar imágenes de productos y mejorar el rendimiento.",
      ...
      return articles
        .filter((article) => {
          ...
          if (filters.difficulty && article.difficulty !== filters.difficulty)
            return false;
          if (filters.maxReadTime && article.readTime > filters.maxReadTime)
            return false;
      ...
      return [
        {
          ...
          answer:
            "Regístrate en KOOWEXA, completa tu perfil de negocio y comienza a agregar productos. Nuestro equipo te guiará en el proceso.",
          category: "tienda",
        },
        {
          question: "¿Qué métodos de pago aceptan?",
          answer:
            "Aceptamos transferencias en CUP, tarjetas nacionales y estamos integrando más opciones de pago para Cuba.",
          category: "pagos",
        },
        {
          question: "¿Cómo contacto con soporte?",
          answer:
            "Puedes crear un ticket de soporte desde tu panel de control o escribirnos a soporte@koowexa.cu",
          category: "soporte",
        },
      ];
    };
