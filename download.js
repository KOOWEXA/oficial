...
class KOOWEXADownload {
    ...
    setupDownloadTracking() {
        ...
        const downloadLinks = document.querySelectorAll('a[download], a[href*=".apk"], a[href*=".exe"], a[href*=".zip"]');
    ...
    trackDownload(fileName, fileUrl) {
        if (window.KOOWEXA_API && window.KOOWEXA_API.getAPI('analytics')) {
            window.KOOWEXA_API.getAPI('analytics').trackEvent('Download', fileName, fileUrl);
        }
